...
def tratamento(sentences):
  ...
  while i >= 0:
    if '/' in sentences[i-1] and sentences[i-1].endswith('a'):
      ...
      new_sentences = sentences[i].split(":")
      elif "//" in new_sentences[1] or not new_sentences[1].strip():  # Se o lado direito de ":" for um URL ou vazio, ignore
        continue
      
      elif new_sentences[1].isupper():  # Se o lado direito de ":" for todo em maiúsculas, é provavelmente uma chave-valor válida
        concatenate.append(sentences[i].strip())
      if temp:
