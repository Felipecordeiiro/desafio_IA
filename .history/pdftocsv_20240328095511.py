...
def tratamento(sentences):
  ...
  while i >= 0:
    ...
    if temp:
      ...
      if len(temp_split) > 1 and "www" not in temp_split[1] and "//" not in temp_split[1] and temp_split[1]:
        concatenate.append(temp_split[0]+':')
        print(sentences[i],sentences[i+1])
        sentences[i] = temp_split[0]+':'
        sentences.insert(i + 1, temp_split[1])
        print(f"posição original {sentences[i]}, um índice a frente {sentences[i+1]}, {sentences[i+2]}")
        break
      else:
        concatenate.append(temp)
    i -= 1
