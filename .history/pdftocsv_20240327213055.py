...
def tratamento(sentences):
  ...
  while i >= 0:
    ...
    if sentences[i][-1] == ':' or ':' in sentences[i]:
      new_sentences = sentences[i].split(":")

      if "//" in new_sentences[1]:
        site = ":".join(new_sentences)
        print(f"essa é a porra do site: {site}")
      elif new_sentences[1].isupper():
         namePreg = ":".join(new_sentences)
         print(f"essa é a porra do nome do pregão: {namePreg}")
      if temp:
