...
def tratamento(sentences):
  ...
  while i >= 0:
    ...
    if not temp_processed and temp:
      ...
      if "Site:" in temp and len(temp.split(" ")) > 1:
        new_temp = temp.split(" ")
        sentences[i] = new_temp[0]
        sentences.insert(i + 1, new_temp[2].strip())
        print(sentences[i+1])

      elif temp.startswith("(http") or temp.startswith("www"):
        #print(temp)
        if temp.find(' ') != -1:
          temp_without_space = temp.split(" ")
          sentences[i] = temp.split(" ")[1]
          print(temp_without_space[1])
