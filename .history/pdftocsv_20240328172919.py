...
def tratamento(sentences):
  ...
  while i >= 0:
    ...
    if sentences[i][-1] == ':' or ':' in sentences[i] or sentences[i].startswith("www"):
      if temp:
    ...
    elif not temp_processed and temp:
      temp_split = temp.split(":")

      if "Site:" in temp and len(temp.split(" ")) > 1:
        new_temp = temp.split(" ")
        sentences[i] = new_temp[0]
        sentences.insert(i + 1, new_temp[1].strip())
