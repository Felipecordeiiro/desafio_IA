...
def train():
    ...
    csvs_paths = [os.path.join(csv_dir,csv_file) for csv_file in csvs_list if]
    ...
    trainer_asks.train([
        ...
        '\n'.join(csvs_files),
        "Que informações disponíveis você tem sobre esses relatórios?",
        '\n'.join(asks_tables)
    ])
    trainer_asks.train([
        "Ajuda",
        ...
        "Sim",
        "Informações gerais sobre um arquivo, como:\n{}\n E informações específicas, como:\n{}".join(asks_tables) .join(asks),
    ])