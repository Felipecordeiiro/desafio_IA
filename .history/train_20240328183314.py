...
def train():
    ...
    asks_company = {
        'Nome do Pregão': 'Quais os nomes de todas as empresas das quais você tem informação?'
    }
    trainer_asks.train(
    ...
    for csv_file_path in csvs_list:
        with open(os.path.join(csv_dir, csv_file_path), 'r', encoding='UTF-8') as csv_file:
            ...
            for column in header:
                for information in column.split(','):
                    if "nome de pregão" in information.lower():
                        empresa = information.split(':')[1].strip().lower()
    ...
    trainer_asks.train([
        "Sobre qual empresa você tem informação?",
        '\n listar todos os Nomes dos Pregões'
        #.join(cod_list),
    ])
    trainer_asks.train([
        "Ajuda",
