...
def tratamento(sentences):
  ...
  while i >= 0:
    ...
    elif 'Nome de Pre' in sentences[i]:
       print(sentences[i])
    ...
    elif 'Balanço Patrimonial - Consolidado' in sentences[i]:
    ...
    elif 'Composição do Capital Social' in sentences[i]:
        sentences[i] = sentences[i] + ' ' +sentences[i+1]
        del sentences[i+1]

    elif 'Ações em Circulação no Mercado' in sentences[i]:
        sentences[i] = sentences[i] + ' - ' + sentences[i+1]
    ...
    elif 'Posição Acionária*' in sentences[i]:
