  ...
  while i >= 0:
    if '/' in sentences[i-1] and sentences[i-1].endswith('a'):
      ...
      if temp:
        new_temp = temp.split(":")
        #print(new_temp)
        if "" != new_temp[1] or new_temp[1].isupper():
          concatenate.append(new_temp[0]+':')
          temp=''
        else:
    ...
    if temp:
      ...
      #print(temp_split)
      if "www" not in temp_split[1] and "//" not in temp_split[1]:
        print(temp_split[1])
      if len(temp_split) > 1 and "www" not in temp_split[1] and "//" not in temp_split[1]:
        #print(temp_split[0])
