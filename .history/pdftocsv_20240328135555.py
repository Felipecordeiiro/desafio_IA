  ...
  while i >= 0:
    ...
    if not temp_processed and temp:
      ...
      if len(temp_split) > 1 and temp_split[1]:
        if "Site" in temp:
           print(f"O índice referente ao elemento site é: {i}")
        if "//" in temp_split[1] and "www" in temp_split[1]: #"//" in temp_split[1] and 
          print(f"atual valor de senteces é:{sentences[i-1]}, índice anterior é {i-1} e tem isso: {sentences[i-1]}. O índice posterior é {i} e tem isso: {sentences[i+1]}")
        else:
          concatenate.add(temp_split[0]+':')
