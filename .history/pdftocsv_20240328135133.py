  ...
  while i >= 0:
    ...
    if not temp_processed and temp:
      ...
      if len(temp_split) > 1 and temp_split[1]:
        if "Site" in temp:
           print(f"O índice do site é: {i}")
        if "//" in temp_split[1] and "www" not in temp_split[1]:
          print(f"atual valor de senteces é:{sentences[i]}, índice anterior é {i-1} e tem o valor {sentences[i-1]} e o índice posterior {sentences[i+1]}")
        else:
